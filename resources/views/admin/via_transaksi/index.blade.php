...
@section('title', 'Data Via Transaksi')
@section('page_title', 'Kelola Via Transaksi')
...
@section('content')
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="min-w-0">
                ...
                <p class="text-xs sm:text-sm text-slate-500 mt-1">
                    Kelola jalur / media transaksi perbankan
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" id="btnOpenAddVia"
                    ...
                    <span class="hidden xs:inline">Tambah Via</span>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div class="p-4 sm:p-5 border-b border-slate-100 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-3">
                <div class="min-w-0 w-full xl:max-w-md flex-1">
                    <h2 class="text-sm font-bold text-slate-800">Daftar Via Transaksi</h2>
                    <p class="text-[10px] sm:text-xs text-slate-400 mt-0.5">
                        Total {{ $vias->total() }} data
                        @if ($search)
                ...
                <form action="{{ route('via.index') }}" method="GET"
                ...
                <table class="w-full text-left text-[10px] sm:text-xs min-w-[640px]">
                    <thead class="bg-slate-50/80 text-slate-500 uppercase tracking-wide text-[10px] sm:text-[11px] font-bold border-b border-slate-100">
                        <tr>
                            ...
                            <th class="py-3 px-4 sm:px-5 whitespace-nowrap w-32">Kode Via</th>
                            <th class="py-3 px-4 sm:px-5 whitespace-nowrap">Nama Via</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        @forelse ($vias as $i => $via)
                            <tr class="hover:bg-slate-50/80 transition group" 
                                data-via-id="{{ $via->id }}"
                                data-via-kode="{{ $via->kode }}"
                                data-via-nama="{{ $via->nama }}">
                                ...
                                <td class="py-3 px-4 sm:px-5 whitespace-nowrap">
                                    @php $jml = $via->transaksi_count ?? 0; @endphp
                                    @if ($jml > 0)
                                ...
                                <td class="py-3 px-4 sm:px-5 whitespace-nowrap text-[10px] sm:text-xs text-slate-500">
                                    <div class="space-y-0.5">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span>{{ $via->created_at ? \Carbon\Carbon::parse($via->created_at)->format('d M Y') : '—' }}</span>
                                        </div>
                                        <div class="flex items-center gap-1.5 text-slate-400">
                                            ...
                                            <span>{{ $via->created_at ? \Carbon\Carbon::parse($via->created_at)->format('H:i') . ' WIB' : '—' }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-3 px-4 sm:px-5 whitespace-nowrap text-right">
                                    <div class="inline-flex items-center gap-1 opacity-70 group-hover:opacity-100 transition">
                                        <button type="button" title="Edit Via"
                                            class="btn-edit-via w-8 h-8 rounded-lg bg-slate-50 hover:bg-blue-50 text-slate-500 hover:text-blue-600 border border-slate-200 hover:border-blue-200 flex items-center justify-center transition">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </button>
                                        <form action="{{ route('via.destroy', $via->id) }}" method="POST"
                                            class="form-delete-via inline-block" 
                                            data-nama="{{ $via->nama }}" 
                                            data-jml="{{ $jml }}">
                                            @csrf
                                            <button type="submit" title="Hapus Via"
                            ...
                            <tr>
                                <td colspan="6" class="py-8 text-center text-slate-400 font-medium">
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        ...
                                        <span>Tidak ada data via transaksi ditemukan.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden transform transition-all">
            <div class="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between gap-3 bg-gradient-to-r from-emerald-50 to-teal-50">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        <h3 id="titleModal" class="text-sm font-bold text-slate-800 truncate">Tambah Via Transaksi</h3>
                        <p id="subtitleModal" class="text-[10px] sm:text-xs text-slate-500 mt-0.5">Tambahkan kode via transaksi baru</p>
                    </div>
                </div>
            ...
            <form id="formVia" action="{{ route('via.store') }}" method="POST" novalidate>
                @csrf
                ...
                <div class="p-4 sm:p-5 space-y-4">
                    ...
                    <div>
                        <label for="inputKode" class="block text-[11px] sm:text-xs font-bold text-slate-700 mb-1.5">
                            Kode Via <span class="text-rose-500">*</span>
                        </label>
                        <div class="relative">
                            <input type="text" name="kode" id="inputKode" required placeholder="cth: 101"
                    ...
                    <div>
                        <label for="inputNama" class="block text-[11px] sm:text-xs font-bold text-slate-700 mb-1.5">
                            Nama Via Transaksi <span class="text-rose-500">*</span>
                        </label>
                        <div class="relative">
                            <input type="text" name="nama" id="inputNama" required placeholder="cth: Teller"
                                class="w-full px-3.5 py-2.5 text-xs sm:text-sm rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition placeholder:text-slate-400">
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 sm:p-5 border-t border-slate-100 bg-slate-50/60 flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2">
                    ...
                    <button type="submit" id="btnSubmitModal"
                        ...
                        <span id="txtBtnSubmit">Simpan Via</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        (function() {
            ...
            const modal = document.getElementById('modalVia');
            const form = document.getElementById('formVia');
            ...
            const btnOpenAdd = document.getElementById('btnOpenAddVia');
            ...
            function openModalForAdd() {
                if (!modal) return;
                if (titleModal) titleModal.textContent = 'Tambah Via Transaksi';
                if (subtitleModal) subtitleModal.textContent = 'Tambahkan kode via transaksi baru';
                if (iconModal) iconModal.innerHTML =
                    '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />';
                if (form) {
                    form.action = '{{ route('via.store') }}';
                    form.dataset.editId = '';
                }
                if (methodInput) methodInput.value = 'POST';
                if (txtBtn) txtBtn.textContent = 'Simpan Via';
                if (inputKode) inputKode.value = '';
                if (inputNama) inputNama.value = '';
                openModal();
            }

            function openModalForEdit(id, kode, nama) {
                if (!modal) return;
                if (titleModal) titleModal.textContent = 'Edit Via Transaksi';
                if (subtitleModal) subtitleModal.textContent = 'Perbarui informasi via transaksi';
                if (iconModal) iconModal.innerHTML =
                    '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />';
                if (form) {
                    form.action = '/via-transaksi/' + encodeURIComponent(id);
                    form.dataset.editId = id;
                }
                if (methodInput) methodInput.value = 'POST';
                if (txtBtn) txtBtn.textContent = 'Perbarui Via';
            ...
            document.querySelectorAll('.btn-edit-via').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    const tr = this.closest('tr[data-via-id]');
                    if (!tr) return;
                    const id = tr.getAttribute('data-via-id');
                    const kode = tr.getAttribute('data-via-kode');
                    const nama = tr.getAttribute('data-via-nama');
                    openModalForEdit(id, kode, nama);
                });
            });
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    if (kode === '') {
                        showError('Kode via wajib diisi.');
                        if (inputKode) inputKode.focus();
                        return;
                    }
                    if (nama === '') {
                        showError('Nama via wajib diisi.');
                    ...
                    fetch(form.action, {
                        ...
                        if (data && data.success) {
                            showToast('success', isEdit ? 'Via Diperbarui' : 'Via Ditambahkan',
                                data.message || (isEdit ? 'Via transaksi berhasil diperbarui.' : 'Via transaksi berhasil ditambahkan.'));
                            setTimeout(function() {
                        ...
                        setLoading(false);
                        let msg = err && err.message ? err.message : 'Gagal menyimpan via transaksi. Silakan coba lagi.';
                        if (err && err.errors) {
                            if (err.errors.kode) {
            ...
            const delForms = document.querySelectorAll('.form-delete-via');
            delForms.forEach(function(f) {
                f.addEventListener('submit', function(e) {
                    if (f.dataset.confirmed === '1') return;
                    e.preventDefault();
                    const nama = f.getAttribute('data-nama') || 'via ini';
                    ...
                    function doConfirm() {
                        ...
                        if (isUsed) {
                            html =
                                ...
                                ' PERINGATAN: VIA TRANSAKSI DIGUNAKAN' +
                                '</p>' +
                                '<p>Via transaksi <strong>' + nama +
                                '</strong> masih digunakan oleh <strong>' + jml +
                                ' riwayat transaksi</strong>.</p>' +
                                '<p>Menghapus via transaksi ini akan gagal. Hapus atau pindahkan data transaksi terkait terlebih dahulu.</p>' +
                                '</div>';
                        } else {
                            html =
                                '<p class="text-sm text-slate-600 mt-1">Via transaksi <strong>' + nama +
                                '</strong> akan dihapus permanen dari sistem.</p>';
                        }
                        window.Swal.fire({
                            title: 'Hapus Via Transaksi?',
                            html: html,
